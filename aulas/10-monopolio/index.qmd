...
::: {.eq-block}
$$W\left({x}_{1},...,{x}_{J},{e}_{1},...,{e}_{J}\right) = \int_{0}^{\sum {x}_{j}} P\left(t\right)dt - \sum_{j=1}^{J} {C}^{j}\left({x}_{j},{e}_{j}\right) - D\left(E\right)$$
:::
...
::: {.diagram}
![](../_assets/figuras/10-otimo.svg){style="max-height:350px"}
:::

- O painel da direita é a figura das aulas 04 a 09: $-C′(E)=(200-E)/3$ cruzando
  $D′(E)=0{,}2E$ em ${E}^{*}=125$, com preço 25.
- [O que esta aula acrescenta é o painel da esquerda. A curva vermelha da
  direita é a curva de abatimento **no nível de produção ${X}^{*}$**. Logo, esta é diferente para cada ${X}^{*}$.]{.fragment fragment-index=1}
...
- [Descer de $\hat{e}=100$ para $e=60$ custa 400 de lucro, que é exatamente a
  área sob a curva marginal.]{.fragment fragment-index=1}
...
- [Na verdade, acima da alíquota de 16,7, a perda sob monopólio desregulado é menor do que a perda com imposto.]{.fragment fragment-index=2}
